...
{
    public class MandateRegOutWardUnProcDataAccessLayer
    {
        Demo2_ZipNach_RBLBank_AngularEntities dbcontext = new Demo2_ZipNach_RBLBank_AngularEntities();
        List<UnderProcessMandatesOutWard> dataList = new List<UnderProcessMandatesOutWard>();
        List<CorporateUnProcOutWard> dataList1 = new List<CorporateUnProcOutWard>();
        List<SponserBankUnProcOutWard> dataList2 = new List<SponserBankUnProcOutWard>();

        public Dictionary<string, object> BankBind(string UserId,string EntityId)
        {
            try
            {
                var result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_Mandate]").With<SponserBankUnProcOutWard>().Execute("@QueryType", "@UserId", "@EntityId", "UserBank", DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(UserId.Replace("_", "%"))), DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(EntityId.Replace("_", "%")))));
        ...
        public Dictionary<string, object> GetUserCorpData(string EntityId)
        {
            try
            {
                var result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_DownloadMandate]").With<CorporateUnProcOutWard>().Execute("@QueryType", "@EntityId", "BindCorporate", DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(EntityId.Replace("_", "%")))));
                //foreach (var dt in Data)
                //{
                //    dataList2 = dt.Cast<SponserBankUnProcOutWard>().ToList();
                //}
        ...
        public IEnumerable<UnderProcessMandatesOutWard> SearchData(string FromDate, string ToDate, string sponserbankcode, string UserId,string EntityId, string Reference, string ActivityID)
        {
            try
            {
                ...
                var Result = dbcontext.MultipleResults("[dbo].[Sp_DownloadMandate]").With<UnderProcessMandatesOutWard>().Execute("@QueryType", "@strToDate", "@strFromDate", "@UserId", "@SponsorBankCode", "@EntityId", "@IsEntity", "@Refrence1", "@ActivityId", "grdMandateDateWiseSubmitted", ToDate, FromDate, DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(UserId.Replace("_", "%"))),sponserbankcode, EntityId, "1", Reference, ActivityID);
                foreach (var dt in Result)
                {
                    dataList = dt.Cast<UnderProcessMandatesOutWard>().ToList();
