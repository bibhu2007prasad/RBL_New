...
{
    public class MandateRegInWardProcDataAccessLayer
    {
        Demo2_ZipNach_RBLBank_AngularEntities dbcontext = new Demo2_ZipNach_RBLBank_AngularEntities();
        List<ProcessMandatesInWard> dataList = new List<ProcessMandatesInWard>();
        List<CorporateProcInWard> dataList1 = new List<CorporateProcInWard>();
        List<SponserBankProcInWard> dataList2 = new List<SponserBankProcInWard>();

        public Dictionary<string, object> BankBind(string UserId, string EntityId)
        {
            try
            {
                var result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_Mandate]").With<SponserBankProcInWard>().Execute("@QueryType", "@UserId", "@EntityId", "UserBank", DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(UserId.Replace("_", "%"))), DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(EntityId.Replace("_", "%")))));
        ...
        public Dictionary<string, object> GetUserCorpData(string EntityId)
        {
            try
            {
                var result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_DownloadMandate]").With<CorporateProcInWard>().Execute("@QueryType", "@EntityId", "BindCorporate", DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(EntityId.Replace("_", "%")))));
                //foreach (var dt in Data)
                //{
                //    dataList2 = dt.Cast<SponserBankUnProcInWard>().ToList();
                //}
        ...
        public IEnumerable<ProcessMandatesInWard> SearchData(string FromDate, string ToDate, string sponserbankcode, string UserId, string EntityId, string Reference, string ActivityID)
        {
            try
            {
