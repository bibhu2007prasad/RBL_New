...
{
    public class MandateRegInWardUnProcDataAccessLayer
    {
        Demo2_ZipNach_RBLBank_AngularEntities dbcontext = new Demo2_ZipNach_RBLBank_AngularEntities();
        List<UnderProcessMandatesInWard> dataList = new List<UnderProcessMandatesInWard>();
        List<CorporateUnProcInWard> dataList1 = new List<CorporateUnProcInWard>();
        List<SponserBankUnProcInWard> dataList2 = new List<SponserBankUnProcInWard>();

        public Dictionary<string, object> BankBind(string UserId,string EntityId)
        {
            try
            {
                var result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_Mandate]").With<SponserBankUnProcInWard>().Execute("@QueryType", "@UserId", "@EntityId", "UserBank", DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(UserId.Replace("_", "%"))), DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(EntityId.Replace("_", "%")))));
                //foreach (var dt in Data)
                //{
                //    dataList2 = dt.Cast<SponserBankUnProcInWard>().ToList();
                //}
                return result;
            }
            catch (Exception ex)
        ...
        public Dictionary<string, object> GetUserCorpData(string EntityId)
        {
            try
            {
                var result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_DownloadMandate]").With<CorporateUnProcInWard>().Execute("@QueryType", "@EntityId", "BindCorporate", DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(EntityId.Replace("_", "%")))));
                //foreach (var dt in Data)
                //{
                //    dataList2 = dt.Cast<SponserBankUnProcInWard>().ToList();
                //}
        ...
        public IEnumerable<UnderProcessMandatesInWard> SearchData(string FromDate, string ToDate, string sponserbankcode, string UserId,string EntityId, string Reference, string ActivityID)
        {
            try
            {
                ...
                var Result = dbcontext.MultipleResults("[dbo].[Sp_Mandate_OnUs]").With<UnderProcessMandatesInWard>().Execute("@QueryType", "@strToDate"
       , "@strFromDate", "@UserId", "@SponsorBankCode", "@EntityID", "@IsEntity", "@Refrence1", "@ActivityId", "grdMandateDateWiseSubmitted", ToDate, FromDate, DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(UserId.Replace("_", "%"))),sponserbankcode, EntityId, "1", Reference, ActivityID);
                foreach (var dt in Result)
                {
                    dataList = dt.Cast<UnderProcessMandatesInWard>().ToList();
