...
{
    public class SubCorporateSetUpDataAccessLayer
    {
        ...
        public Dictionary<string, object> BindUtility()
        {
            try
            {
                var Result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_BankMaster]").With<UtilityCodeMaster>().Execute("@QueryType", "BindUtilityCode"));
                return Result;
            }
        ...
        public Dictionary<string, object> BindCategoryWithRelated()
        {
            try
            {
                var Result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_BankMaster]").With<CategoryCodeMaster>().With<RegionalMaster>().With<PSMMaster>().With<BussinessSegmentMaster>().With<SettlementTypeMaster>().Execute("@QueryType", "BindCategoryCode"));
                return Result;
            }
        ...
        public Dictionary<string, object> BindCountryAndBank()
        {
            try
            {
                var Result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_BankMaster]").With<BindCountry>().With<BindBank>().With<EntityBusinessCode>().Execute("@QueryType", "BindCountry"));
                return Result;
            }
        ...
        public Dictionary<string, object> BindGridDataAccess(string UserId, string EntityId)
        {
            try
            {
                var Result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_BankMaster]").With<MainGrid>().Execute("@QueryType", "@UserId", "@EntityId", "BindEntity", DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(UserId.Replace("_", "%"))), DbSecurity.Decrypt(HttpContext.Current.Server.UrlDecode(EntityId.Replace("_", "%")))));
                return Result;
            }
        ...
        public Dictionary<string, object> SaveDataDataAccess(AllFieldOfForm allFieldOfForm, string EntityId = "")
        {
            try
            {
...

                {
                    ...
                    var Result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_BankMaster]").With<CommonClass>().Execute(
"@QueryType", "@InstructingMemberId", "@XmlSponsorBankCode", "@Type", "@Code", "@Name", "@ContactPerson","@SponsorBankCode", "@UtilityCode", "@SponsorBankName", "@UserId", "@Addr1", "@Email", "@CountryId", "@CityId", "@StateId",
...
"@IsShowMandateMode", "@IsSendEmailCustomer", "@IschkEmandate", "@IschkPhysical", "@Xmldebittype", "@XmlToDebit", "@XmlType", "@Xmlfrequency", "@chkIsRefrence2Mandatory", "@dtBankAmount", "@IsThirdTransaction", "@chkIsZipSure", "@chkIsAllowEManadte", "@ISTodateMandatoryEnach", "@APPId", "@XmlFileName", "@AccountNumber", "@ReCheck", "@CheckerRequire", "@XmlModeType", "@XmlCategoryCode", "@PAN", "@TAN", "@GSTIN", "@IsNachPresentment", "@BillingAddress", "@ChargeDebitAcNo", "@BankCorporateId", "@BranchName", "@BranchCode", "@IFSC", "@RegionalManagerId",
"@PSM", "@BillingContactPerson", "@Designation", "@ContactNo", "@FaxNo", "@ArrangementInDays", "@PickUpLocationAddress", "@PickUpContactPerson", "@CommunicationMailPrimary", "@Ip", "@Url", "@h2husername", "@h2hpassword", "@dtBillingMail", "@CorporateId", "@XmlFileNameImgDOC", "@SettlementTypeId", "@BusinessSegmentId", "@dtcontactperson",
...
, ip, url, h2husername, h2hpassword, dtBilligMail, CorporateId, XmlFileNameImgDOC, settlementtype, businesssegment, dtcontactperson.ToString()));
                    return Result;
                }
                else
                {
...
"@Mobile", "@Pincode", "@ImagePath", "@UserName", "@password", "@passwordKey", "@DebitType", "@FrequencyType", "@ToDebit", "@Amount", "@Activate", "@XmlPaymentMode", "@EntityBusinessCode", "@IsOverMandate",
"@IsRefrenceCheck", "@IsValidationCountEnable", "@BankValidationAdminCount", "@BankValidationUserCount", "@AcValidationAdminCount", "@AcValidationUserCount", "@IsRefNumerc",
...
, ip, url, h2husername, h2hpassword, dtBilligMail, CorporateId, XmlFileNameImgDOC,settlementtype, businesssegment, dtcontactperson.ToString()));
        ...
        public Dictionary<string, object> EditEntity(string EntityId)
        {
            try
            {
                var Result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_BankMaster]").With<Table>().With<Table1>().With<Table2>().With<Table3>().With<Table4>().With<Table5>().With<Table6>().With<Table7>().With<Table8>().With<Table9>().With<Table10>()
        ...
        public Dictionary<string, object> BindStateDataAccess(string CountryId)
        {
            try
            {
                var Result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_BankMaster]").With<BindState>().Execute("@QueryType", "@CountryId", "Bindstate", CountryId));
                return Result;
            }
        ...
        public Dictionary<string, object> BindCityDataAccess(string StateId)
        {
            try
            {
                var Result = Common.Getdata(dbcontext.MultipleResults("[dbo].[Sp_BankMaster]").With<BindCity>().Execute("@QueryType", "@StateId", "BindCity", StateId));
                return Result;
            }
